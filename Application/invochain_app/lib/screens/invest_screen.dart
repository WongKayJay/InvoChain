...
class _InvestScreenState extends State<InvestScreen> with SingleTickerProviderStateMixin {
  ...
  List<InvestmentOpportunity> _getFilteredOpportunities(AppDataProvider appData) {
    var filtered = appData.allOpportunities.where((opp) {
      // Search filter
  ...
  Widget build(BuildContext context) {
    ...
    final filteredOpportunities = _getFilteredOpportunities(appData);
    ...
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
          _buildOpportunitiesTab(theme, filteredOpportunities),
          _buildOnChainTrackingTab(theme, appData),
        ],
      ),
    );
  }

  Widget _buildOpportunitiesTab(ThemeData theme, List<InvestmentOpportunity> filteredOpportunities) {
    return Column(
      children: [
        // Search Bar
        Padding(
  ...
  Widget _buildOnChainTrackingTab(ThemeData theme, AppDataProvider appData) {
    final myInvestments = appData.userInvestments;
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () {
                ...
                if (amount > 0 && amount >= (opportunity.amount * 0.01)) {
                  // Add investment through provider
                  context.read<AppDataProvider>().addInvestment(opportunity, amount);
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('Minimum investment is \$${(opportunity.amount * 0.01).toStringAsFixed(2)}'),
